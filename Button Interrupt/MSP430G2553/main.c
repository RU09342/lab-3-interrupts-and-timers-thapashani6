...
void main(void)
{
  ...
  //PM5CTL0 &= ~LOCKLPM5;        // Disable the GPIO power-on default High-Z mode
    ...
    P1IE  |= BIT3;               // Interrupt Enable on Port 1;
    P1IES &= ~BIT3;               // Interrupt Edge Select, from Low -> High, when button is pressed
    P1IFG &= ~BIT3;              // Clear Interrupt Flag

    P1REN |= BIT3;  // enable resistor on the switch P1.1
    P1OUT |= BIT3;  // set resistor to pull up on switch P1.1
...
__interrupt void Port_1(void)
{
    ...
    P1IFG &= ~BIT3;            // Clear Interrupt flag at the end of the ISR
}
